...
class StartHandler(
    ...
    private fun getPaginateOrganizations(paginate: Paginate = Paginate(0), userId: Long): InlineKeyboardMarkup {
        ...
        if (organizations.hasPrevious()) {
            navigationButtons.add("<-" to CallbackData(CallbackQueryEnum.PAGINATE_ORGANIZATIONS, Paginate(paginate.decreasePage())))
        }
        if (organizations.hasNext()) {
            navigationButtons.add("->" to CallbackData(CallbackQueryEnum.PAGINATE_ORGANIZATIONS, Paginate(paginate.increasePage())))
        }

        return  UpdateUtil.createInlineKeyboard(
